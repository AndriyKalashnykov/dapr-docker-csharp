...
**Runtime surface:** ASP.NET Core minimal-API subscriber wired to a Dapr sidecar for pub/sub (Redis Streams) and state (Redis), with Jaeger declared for OTel traces. **Delivery surface:** TUnit + FakeItEasy + `WebApplicationFactory` test pyramid, `dotnet format --verify-no-changes` and `-warnaserror` gate composed into `make static-check`, NuGet `--vulnerable` audit, GitHub Actions CI on a `global.json`-pinned SDK with `packages.lock.json` caching, Renovate-managed deps with branch automerge, mise-pinned auxiliary toolchain.
